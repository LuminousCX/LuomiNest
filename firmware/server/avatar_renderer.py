...
DEVICE_PROFILES = {
    "s3": {"width": 128, "height": 160},
    "p4": {"width": 400, "height": 540},
}
...
def apply_device_profile(device: str):
    global WIDTH, HEIGHT, FACE_CX, FACE_CY, FACE_R
    global EYE_Y, EYE_R, L_EYE_X, R_EYE_X
    global MOUTH_Y, BLUSH_Y, BLUSH_R, L_BLUSH_X, R_BLUSH_X, BROW_Y

    profile = DEVICE_PROFILES.get(device, DEVICE_PROFILES["s3"])
    WIDTH = profile["width"]
    ...
    FACE_CX = int(64 * scale_x)
    FACE_CY = int(82 * scale_y)
    FACE_R = int(46 * scale)

    EYE_Y = int(70 * scale_y)
    EYE_R = int(7 * scale)
    L_EYE_X = int(46 * scale_x)
    R_EYE_X = int(82 * scale_x)

    MOUTH_Y = int(102 * scale_y)
    BLUSH_Y = int(90 * scale_y)
    BLUSH_R = int(6 * scale)
    L_BLUSH_X = int(36 * scale_x)
    R_BLUSH_X = int(92 * scale_x)

    BROW_Y = int(58 * scale_y)
...
def rgb888_to_rgb565_le(img: Image.Image) -> bytes:
    ...
    r = ((arr[:, :, 0].astype(np.uint16) + 4) >> 3) << 11
    g = ((arr[:, :, 1].astype(np.uint16) + 2) >> 2) << 5
    b = (arr[:, :, 2].astype(np.uint16) + 4) >> 3
    rgb565 = r | g | b
    return rgb565.astype(np.dtype("<u2")).tobytes()


def rgb888_to_bgr565_le(img: Image.Image) -> bytes:
    import numpy as np
    arr = np.array(img, dtype=np.uint8)
    b = ((arr[:, :, 2].astype(np.uint16) + 4) >> 3) << 11
    g = ((arr[:, :, 1].astype(np.uint16) + 2) >> 2) << 5
    r = (arr[:, :, 0].astype(np.uint16) + 4) >> 3
    bgr565 = b | g | r
    return bgr565.astype(np.dtype("<u2")).tobytes()


def rgb888_to_rgb565_be(img: Image.Image) -> bytes:
    import numpy as np
    arr = np.array(img, dtype=np.uint8)
    r = ((arr[:, :, 0].astype(np.uint16) + 4) >> 3) << 11
    g = ((arr[:, :, 1].astype(np.uint16) + 2) >> 2) << 5
