...
#define AP_SSID_PREFIX  "LuomiNest-"
...
".scan-item .lock{font-size:12px;color:#faad14}"
...
"<span style='color:#1677ff'>Scan for WiFi networks</span></div>"
"</div>"
"</div>"
"<div class='form-group'>"
"<label>SSID</label>"
"<input id='ssid' placeholder='Enter SSID or select from scan'>"
"</div>"
"<div class='form-group'>"
...
"<input id='broker' placeholder='mqtt://192.168.1.222:1883'>"
"</div>"
"<div class='form-group'>"
"<label>Client ID</label>"
"<input id='client' placeholder='luominest_s3_01'>"
"</div>"
...
"+'<span>'+(n.auth>0?'<span class=\"lock\">&#128274;</span> ':'')"
"+'<span class=\"rssi\">'+n.rssi+' dBm</span></span></div>'"
"});"
...
"if(d.ok){showStatus('wifiStatus','Saved! Restarting device...','success');"
...
".then(r=>r.json()).then(d=>{"
"if(d.ok){showStatus('serverStatus','Saved! Restarting device...','success');"
"setTimeout(function(){fetch('/api/restart',{method:'POST'})},1500)}"
...
"+'<div class=\"info-row\"><span class=\"k\">MQTT Client</span>"
"<span class=\"v\">'+(d.mqtt_client||'N/A')+'</span></div>'"
...
"+'<div class=\"info-row\"><span class=\"k\">Uptime</span>"
"<span class=\"v\">'+d.uptime+'s</span></div>';"
...
static esp_err_t handle_captive_detect(httpd_req_t *req)
{
...
static httpd_handle_t start_webserver(void)
{
    ...
    if (httpd_start(&s_server, &config) == ESP_OK) {
        ...
        httpd_uri_t uri_config = { .uri = "/api/config", .method = HTTP_POST, .handler = handle_config };
        httpd_uri_t uri_restart = { .uri = "/api/restart", .method = HTTP_POST, .handler = handle_restart };

        httpd_uri_t uri_cp_apple = { .uri = "/generate_204", .method = HTTP_GET, .handler = handle_captive_detect };
        httpd_uri_t uri_cp_ms = { .uri = "/connecttest.txt", .method = HTTP_GET, .handler = handle_captive_detect };
        httpd_uri_t uri_cp_kindle = { .uri = "/kindle-wifi/wifistub.html", .method = HTTP_GET, .handler = handle_captive_detect };
        ...
        httpd_register_uri_handler(s_server, &uri_cp_apple);
        httpd_register_uri_handler(s_server, &uri_cp_ms);
        httpd_register_uri_handler(s_server, &uri_cp_kindle);
...
esp_err_t web_config_load(ln_config_t *cfg)
{
    ...
    nvs_get_str(handle, LN_NVS_KEY_WIFI_SSID, cfg->wifi_ssid, &len);

    len = sizeof(cfg->wifi_pass);
    nvs_get_str(handle, LN_NVS_KEY_WIFI_PASS, cfg->wifi_pass, &len);

    len = sizeof(cfg->mqtt_broker);
    nvs_get_str(handle, LN_NVS_KEY_MQTT_BROKER, cfg->mqtt_broker, &len);

    len = sizeof(cfg->mqtt_client);
    nvs_get_str(handle, LN_NVS_KEY_MQTT_CLIENT, cfg->mqtt_client, &len);
...
esp_err_t web_config_save(const ln_config_t *cfg)
{
    ...
    ESP_LOGI(TAG, "Config saved: SSID=%s, Broker=%s, Client=%s",
             cfg->wifi_ssid, cfg->mqtt_broker, cfg->mqtt_client);
    return ESP_OK;
}
...
{
    nvs_handle_t handle;
    if (nvs_open(LN_NVS_NAMESPACE, NVS_READONLY, &handle) != ESP_OK) {
        return false;
    }
    size_t len = 0;
...
esp_err_t web_config_start_ap(void)
{
    ...
    char ap_ssid[32];
    snprintf(ap_ssid, sizeof(ap_ssid), AP_SSID_PREFIX "%02X%02X%02X",
             mac[3], mac[4], mac[5]);
    ...
    esp_netif_get_ip_info(esp_netif_get_handle_from_ifkey("ap0"), &ip_info);
