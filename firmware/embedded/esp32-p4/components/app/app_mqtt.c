 ...
 * 从旧 esp32-p4/main/app_mqtt.c 移植, 代码基本不变
...
esp_err_t app_mqtt_init(const char *broker_uri, const char *client_id)
{
    ...
    ESP_LOGI(TAG, "MQTT init: broker=%s, client_id=%s", broker_uri, client_id);
    ...
    esp_err_t start_ret = esp_mqtt_client_start(s_client);
    if (start_ret != ESP_OK) {
        ESP_LOGE(TAG, "MQTT client start failed: %s", esp_err_to_name(start_ret));
