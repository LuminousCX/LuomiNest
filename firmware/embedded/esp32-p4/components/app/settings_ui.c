/**
 * LuomiNest P4 - 设置页面 UI
 * 从旧 esp32-p4/main/settings_ui.c 移植, 适配新架构:
 *   - 亮度控制走 bsp_lcd_set_brightness() 而非 mipi_lcd_brightness_set()
 *   - WiFi 扫描用 #if CONFIG_ESP_WIFI_ENABLED 保护 (P4 无 WiFi 时隐藏)
 *   - NVS 配置走 web_config.h
 */
...
static void save_btn_cb(lv_event_t *e)
{
    ...
    bsp_lcd_set_brightness(cfg.brightness);
    ...
    if (ret == ESP_OK) {
        ...
        /* 用 lv_timer 替代 vTaskDelay, 不阻塞 LVGL task */
        lv_timer_t *restart_timer = lv_timer_create(restart_timer_cb, 1500, NULL);
...
esp_err_t settings_ui_init(settings_ui_t *s, lv_obj_t *parent, int32_t width, int32_t height)
{
    ...
    /* WiFi section (only available when ESP WiFi is enabled) */
#if CONFIG_ESP_WIFI_ENABLED
    ...
    lv_obj_set_style_radius(s->wifi_scan_btn, 0, 0);
    ...
    lv_obj_set_style_radius(kb_close, 0, 0);
    ...
    lv_obj_set_style_radius(s->kb, 0, LV_PART_ITEMS);
