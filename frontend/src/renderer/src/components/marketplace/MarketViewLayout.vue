...
import MarketplaceSearch from '../marketplace/MarketplaceSearch.vue'
import MarketplaceCategories from '../marketplace/MarketplaceCategories.vue'
import MarketplaceFilters from '../marketplace/MarketplaceFilters.vue'
import MarketplaceCard from '../marketplace/MarketplaceCard.vue'
import MarketplaceBanner from '../marketplace/MarketplaceBanner.vue'
...
const categories = computed(() => store.getCategories(props.marketType))
const activeCategory = ref('all')
const showFilters = ref(false)

const filter = computed(() =>
  props.marketType === 'plugin' ? store.pluginFilter : store.skillFilter
)

const filteredItems = computed(() => {
  const items = props.marketType === 'plugin'
    ? store.filteredPluginItems
    : store.filteredSkillItems
...
const featuredItems = computed(() =>
  props.marketType === 'plugin' ? store.featuredPlugins : store.featuredSkills
)
...
function updateFilter(updates: Partial<MarketplaceFilter>) {
  store.setFilter(props.marketType, updates)
}
        ...
        <div class="items-section">
          <div class="section-header">
            <h3 class="section-title">
              {{ activeCategory === 'all' ? allLabel : categories.find(c => c.id === activeCategory)?.name || title.replace('市场', '') }}
            </h3>
