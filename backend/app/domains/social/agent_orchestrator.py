...
class AgentOrchestrator:
    ...
    async def orchestrate_stream(
        ...
        try:
            ...
            async for event in self._execute_sub_tasks_stream(sub_tasks, group, session):
                yield event
    ...
    async def _analyze_with_coordinator(
        ...
        provider = coordinator.get("provider") or llm_adapter.default_provider
        model = coordinator.get("model")
    ...
    async def _execute_sub_tasks(
        ...
        if all(t.depends_on for t in sub_tasks):
            execution_order = "serial"

        if execution_order == "parallel":
            independent = [t for t in sub_tasks if not t.depends_on]
            dependent = [t for t in sub_tasks if t.depends_on]
            ...
            for task in dependent:
                await self._execute_single_task(task, group, session)
        else:
            for task in sub_tasks:
                await self._execute_single_task(task, group, session)

    async def _execute_sub_tasks_stream(
        self,
        sub_tasks: list[SubTask],
        group: dict,
        session: CollaborationSession,
        ...
        if independent:
            ...
            async def run_task(task: SubTask):
                ...
                async for event in self._execute_single_task_stream(task, group, session):
                    events.append(event)
                task_events[task.task_id] = events

            async def stream_task(task: SubTask):
                async for event in self._execute_single_task_stream(task, group, session):
                    yield event

            running_tasks = []
            for task in independent:
                running_tasks.append(asyncio.create_task(run_task(task)))

            yield {
                "type": "tasks_started",
                "data": {
                    "task_ids": [t.task_id for t in independent],
                    "parallel": True,
                },
            }
    ...
    async def _execute_single_task(
        ...
        try:
            ...
            if not agent:
                member = None
                for m in group.get("members", []):
                    if m.get("type") == "agent" and m.get("agent_id") != _find_coordinator_agent(group)["id"]:
...
{role_prompt}"""
    ...
    async def _save_messages(self, group: dict, session: CollaborationSession) -> None:
        ...
        group["messages"].append(user_msg)
        ...
        if session.phase == CollaborationPhase.COMPLETED and session.sub_tasks:
            for task in session.sub_tasks:
                if task.status == TaskStatus.COMPLETED and task.result:
                    ...
                    group["messages"].append(task_msg)
            ...
            if session.final_result:
                ...
                group["messages"].append(synthesis_msg)
            ...
            group["messages"].append(direct_msg)
            ...
            group["messages"].append(error_msg)
