...
async def _download_model(target_dir: Path) -> None:
    """自动下载并解压 vits-melo-tts-zh_en 模型（异步版本）.
...
def _download_model_sync(target_dir: Path) -> None:
    """自动下载并解压 vits-melo-tts-zh_en 模型（同步版本，用于 __init__）.
    ...
    for attempt in range(1, _MAX_RETRIES + 1):
        try:
            _download_and_extract_once_sync(target_dir)
            logger.info(f"[SherpaOnnxTTS] 模型下载完成（第 {attempt} 次尝试）")
            return
        except Exception as e:
            last_error = e
...
def _download_and_extract_once_sync(target_dir: Path) -> None:
    """执行一次下载+解压流程（同步版本）."""
    with tempfile.TemporaryDirectory(prefix="sherpa_tts_") as tmp_dir:
        tmp_path = Path(tmp_dir)
        archive_path = tmp_path / "vits-melo-tts-zh_en.tar.bz2"
        ...
        with httpx.Client(timeout=_DOWNLOAD_TIMEOUT, follow_redirects=True) as client:
            with client.stream("GET", _MODEL_DOWNLOAD_URL) as resp:
                resp.raise_for_status()
                total = int(resp.headers.get("content-length", 0))
                downloaded = 0
                last_log_pct = 0
...
class SherpaOnnxTTSProvider(TTSProvider):
    ...
    def __init__(self, model_dir: str | Path | None = None, num_threads: int = 2, speed: float = 1.0):
        ...
        # 模型不存在时自动下载（同步，避免事件循环冲突）
        model_path = model_dir / "model.onnx"
        if not model_path.exists():
            logger.info(f"[SherpaOnnxTTS] 模型未找到，开始自动下载...")
            _download_model_sync(model_dir)
