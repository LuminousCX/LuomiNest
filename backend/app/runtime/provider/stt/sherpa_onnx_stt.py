...
async def _download_sense_voice_model(target_dir: Path) -> None:
    ...
    await download_and_extract_model(
        _SENSE_VOICE_URL,
        target_dir,
        log_prefix="[SherpaOnnxSTT]",
        download_label="SenseVoice 模型",
        size_hint="约 220MB",
        timeout=settings.MODEL_DOWNLOAD_TIMEOUT,
        tmp_prefix="sherpa_stt_",
        archive_name="sense-voice.tar.bz2",
        expected_files=("model.int8.onnx", "model.onnx"),
        missing_error=lambda d: f"解压后未找到模型文件: {d}",
    )
...
class SherpaOnnxSTTProvider(BaseSTTProvider):
