...
使用 luominest_config_store（SQLite config_items 表）持久化插件启用/禁用状态，
...
class CxPluginLifecycle:
    ...
    def _read_disabled_plugins(self) -> list[str]:
        ...
        return _normalize_disabled_plugins(luominest_config_store.get(_DB_KEY))

    def _write_disabled_plugins(self, disabled: list[str]) -> None:
        """写入 config_items（唯一权威源）。"""
        luominest_config_store.set(_DB_KEY, disabled)
    ...
    def _merge_legacy_json(self) -> None:
        ...
        try:
            ...
            if legacy_ids:
                existing = _normalize_disabled_plugins(luominest_config_store.get(_DB_KEY))
    ...
    async def enable_plugin(self, plugin_id: str) -> bool:
        ...
        async with self._lock:
            ...
            luominest_plugin_registry.update_status(plugin_id, CxPluginStatus.ENABLED)
    ...
    async def reload_plugin(self, plugin_id: str) -> bool:
        """重载插件 — 卸载后重新加载。"""
        async with self._lock:
            await luominest_plugin_loader.unload_single(plugin_id)
            metadata = luominest_plugin_registry.get_plugin(plugin_id)
            plugin_dir = metadata.plugin_dir if metadata else None
            if plugin_dir is None:
                return False
            return await luominest_plugin_loader.load_single(plugin_dir)

    async def unload_plugin(self, plugin_id: str) -> bool:
        """卸载插件。"""
        async with self._lock:
            return await luominest_plugin_loader.unload_single(plugin_id)

    async def reload_all(self) -> int:
        """重载所有插件。"""
        async with self._lock:
            loaded_ids = luominest_plugin_loader.get_loaded_ids()
            for plugin_id in list(loaded_ids):
                await luominest_plugin_loader.unload_single(plugin_id)
            luominest_plugin_registry.clear()
            return await luominest_plugin_loader.load_all()
...
# 全局单例
luominest_plugin_lifecycle = CxPluginLifecycle()
