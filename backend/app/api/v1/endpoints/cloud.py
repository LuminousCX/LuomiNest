...
- PUT /cloud/token：注入 {accessToken, cloudBaseUrl, routingMode} → CloudTokenStore（仅内存）
...
async def set_cloud_token(req: CloudTokenRequest):
    ...
    cloud_token_store.set(req.access_token, req.cloud_base_url, req.routing_mode)
    logger.info(
        f"[Cloud] Token injected: mode={req.routing_mode}, baseUrl={req.cloud_base_url}, "
        f"token=***{req.access_token[-4:]}"
    )
